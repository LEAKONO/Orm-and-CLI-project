...
class Product:
    ...
    def create(cls, name, price, category_id):
        ...
        CONN.commit()
        return cls(CURSOR.lastrowid, name, price, category_id)
    ...
    def update(self):
        CURSOR.execute('UPDATE products SET name = ?, price = ?, category_id = ? WHERE id = ?', (self.name, self.price, self.category_id, self.id))
        CONN.commit()
